...
import { getLogger } from 'omelop-logger'; import { Application } from '../application';
...
export function loadDefaultComponents(app: Application) {
    ...
    if (app.serverType === Constants.RESERVED.MASTER) {
        app.load(omelop.components.master, app.get('masterConfig'));
    } else {
        app.load(omelop.components.proxy, app.get('proxyConfig'));
        if (app.getCurServer().port) {
            app.load(omelop.components.remote, app.get('remoteConfig'));
        }
        if (app.isFrontend()) {
            app.load(omelop.components.connection, app.get('connectionConfig'));
            app.load(omelop.components.connector, app.get('connectorConfig'));
            app.load(omelop.components.session, app.get('sessionConfig'));
            // compatible for schedulerConfig
            if (app.get('schedulerConfig')) {
                app.load(omelop.components.pushScheduler, app.get('schedulerConfig'));
            } else {
                app.load(omelop.components.pushScheduler, app.get('pushSchedulerConfig'));
            }
        }
        app.load(omelop.components.backendSession, app.get('backendSessionConfig'));
        app.load(omelop.components.channel, app.get('channelConfig'));
        app.load(omelop.components.server, app.get('serverConfig'));
    }
    app.load(omelop.components.monitor, app.get('monitorConfig'));
}
