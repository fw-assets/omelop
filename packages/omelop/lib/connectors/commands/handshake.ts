...
export class HandshakeCommand {
    ...
    handle(socket: ISocket, msg: any) {
        ...
        if (this.useDict) {
            let dictVersion = omelop.app.components.__dictionary__.getVersion();
            if (!msg.sys.dictVersion || msg.sys.dictVersion !== dictVersion) {

                // may be deprecated in future
                opts.dict = omelop.app.components.__dictionary__.getDict();

                // 用不到这个。
                //    opts.routeToCode = omelop.app.components.__dictionary__.getDict();
                //     opts.codeToRoute = omelop.app.components.__dictionary__.getAbbrs();
        ...
        if (this.useProtobuf) {
            let protoVersion = omelop.app.components.__protobuf__.getVersion();
            if (!msg.sys.protoVersion || msg.sys.protoVersion !== protoVersion) {
                opts.protos = omelop.app.components.__protobuf__.getProtos();
            }
            opts.useProto = true;
        }

        if (!!omelop.app.components.__decodeIO__protobuf__) {
            if (!!this.useProtobuf) {
                throw new Error('protobuf can not be both used in the same project.');
            }
            let component = omelop.app.components.__decodeIO__protobuf__ as any;
        ...
        if (this.useCrypto) {
            omelop.app.components.__connector__.setPubKey(socket.id, msg.sys.rsa);
        }
