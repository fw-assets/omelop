...
import { getLogger, ILogger } from 'omelop-logger';
...
import { IModule, IModuleFactory } from 'omelop-admin';
...
import { MailStationErrorHandler, RpcFilter, RpcMsg } from 'omelop-rpc';
...
export class Application {
     ...
     *
     * @param {Object} logger omelop-logger instance without configuration
     *
     * @memberOf Application
     */
    ...
    beforeStopHook(fun: BeforeStopHookFunction) {
        logger.warn('this method was deprecated in omelop 0.8');
    ...
    stop(force: boolean) {
        if (this.state > STATE_STARTED) {
            logger.warn('[omelop application] application is not running now.');
            return;
        }
