...
import { RpcClient, createClient } from 'omelop-rpc';
import * as pathUtil from '../util/pathUtil';
import * as Constants from '../util/constants';
import { getLogger } from 'omelop-logger';
import { Application } from '../application';
import { IComponent } from '../interfaces/IComponent';
import { RpcClientOpts } from 'omelop-rpc';
...
export class ProxyComponent implements IComponent {
    ...
    start(cb: (err?: Error) => void) {
        if (this.opts.enableRpcLog) {
            logger.warn('enableRpcLog is deprecated in 0.8.0, please use app.rpcFilter(omelop.rpcFilters.rpcLog())');
        }
