/*!
 * Omelop -- consoleModule watchServer
...
import { IModule, MonitorCallback, MasterCallback, ModuleType, MonitorAgent, MasterAgent } from 'omelop-admin';
...
function dumpCPU(handle: HandleType, _agent: MonitorAgent | MasterAgent, comd: string, context: string, param: { times: number, filepath: string, force: boolean }, cb: (err?: Error | string, data?: any) => void) {
    if (handle === 'client') {
        ...
        cb(null, 'cpu dump is unused in 1.0 of omelop');
        /**
    ...
    if (handle === 'client') {
        ...
        checkFilePath(filepath, force, function (err: string) {
            ...
            try {
                ...
                cb('omelop-admin require heapdump');
            }
        });
    }
}
