...
let logger = getLogger('omelop-rpc', 'rpc-client');
...
export class RpcClient {
    ...
    start(cb: (err?: Error) => void) {
        ...
        this._station.start(function (err: Error) {
            if (err) {
                logger.error('[omelop-rpc] client start fail for ' + err.stack);
                return cb(err);
            }
    ...
    rpcInvoke(serverId: string, msg: RpcMsg, cb: (err: Error, ...args: any[]) => void) {
        ...
        if (this.state !== STATE_STARTED) {
            ...
            logger.error('[omelop-rpc] fail to do rpc invoke for client is not running');
            cb ? cb(new Error('[omelop-rpc] fail to do rpc invoke for client is not running')) : null;
            return;
        }
    ...
    private rpcToRoute(routeParam: any, serviceName: string, methodName: string, args: Array<any>, attach: RemoteServerCode, notify: boolean = false) {
        if (this.state !== STATE_STARTED) {
            return Promise.reject(new Error('[omelop-rpc] fail to invoke rpc proxy for client is not running'));
        }
    ...
    private rpcToSpecifiedServer(serverId: string, serviceName: string, methodName: string, args: Array<any>, attach: RemoteServerCode, notify: boolean = false) {
        ...
        return new Promise<any>((resolve, reject) => {
            if (typeof serverId !== 'string') {
                logger.error('[omelop-rpc] serverId is not a string : %s', serverId);
                return;
            }
            ...
            if (serverId === '*') {
                ...
                if (!servers) {
                    logger.error('[omelop-rpc] serverType %s servers not exist', serverType);
                    return;
                }
        ...
        return (function (): Proxy {
            ...
            let proxy: any = function () {
                ...
                if (len < 1) {
                    ...
                    logger.error('[omelop-rpc] invalid rpc invoke, arguments length less than 1, namespace: %j, serverType, %j, serviceName: %j, methodName: %j',
                        attach.namespace, attach.serverType, serviceName, methodName);
                    return Promise.reject(new Error('[omelop-rpc] invalid rpc invoke, arguments length less than 1'));
                }
    ...
    private getRouteFunction(): TargetRouterFunction {
        if (!!this.routerType) {
            ...
            if (typeof this.router === 'function') {
                ...
                logger.error('[omelop-rpc] invalid route function.');
                return;
            }
