[![Build Status](https://travis-ci.org/node-omelop/omelop-admin.svg?branch=master)](https://travis-ci.org/node-omelop/omelop-admin)

#omelop-admin

`omelop-admin` is an admin console library for [omelop](https://github.com/NetEase/omelop). It provides the a series of utilities to monitor the `omelop` server clusters.

##Installation

```
npm install omelop-admin
```
...
There are three process roles in `omelop-admin`: master, monitor and client.
...
+ client - `omelop-admin` client process that fetches the status from master server, such as [omelop-admin-web](https://github.com/NetEase/omelop-admin-web) and [omelop-cli](https://github.com/NetEase/omelop-cli).
...
Main service of `omelop-admin` that runs in both master and monitor processes. It maintains the master agent or monitor agent for the process, loads the registed modules and provides the messages routing service for the messages from other processes.

###MasterAgent  

`omelop-admin` agent that runs on the master process to provide the basic network communication and protocol encoding and decoding.

###MonitorAgent  

`omelop-admin` agent that runs on the monitor process to provide the basic network communication and protocol encoding and decoding. 

###Module  
 
Module is the place to implement the monitor logic, such as process status collecting. Developer can register modules in `omelop-admin` to customize all kinds of system monitors.
...
<center>
![omelop-admin-arch](http://omelop.netease.com/resource/documentImage/omelop-admin-arch.png)
</center>

##Usage

```javascript
var admin = require("omelop-admin");
```
...
Module.prototype.monitorHandler = function(agent, msg) {
  var word = agent.id + ' hello omelop';
...
you must register your customized modules to omelop to make it work.  
...
##User level control  
omelop-admin defines user level for admin client to login master server in this schema  
```javascript
{
    "id": "user-1",
    "username": "admin",
...
##Self-defined auth 
omelop-admin provides a simple auth function in [omelop-admin auth](https://github.com/NetEase/omelop-admin/blob/master/lib/util/utils.js#L78)  
developers can provide self-defined auth in omelop by  
in master server
```javascript
...
omelop-admin provides a simple auth function in [omelop-admin auth](https://github.com/NetEase/omelop-admin/blob/master/lib/util/utils.js#L117)  
developers can provide self-defined auth in omelop by  
in master server
```javascript
...
when using in omelop, you should fill all your servers with type:token  

###Notes  

`omelop-admin` provides a series of useful system modules by default. But most of them are turned off by default. Add a simple line of code in `app.js` as below to enable them.
