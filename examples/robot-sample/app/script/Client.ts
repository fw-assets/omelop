import * as omelop from './PinusForEgret';
import { Actor } from 'omelop-robot';
...
export class Client {
    ...
    pinusClient = new omelop.WSClient();

    public connectGate(): void {

        let host = '127.0.0.1';
        let port = '3014';
        this.pinusClient.on(omelop.WSClient.EVENT_IO_ERROR, function (event) {
            // 错误处理
            console.error('error', event);
        });
        this.pinusClient.on(omelop.WSClient.EVENT_CLOSE, function (event) {
            // 关闭处理
            console.error('close', event);
        });
        this.pinusClient.on(omelop.WSClient.EVENT_HEART_BEAT_TIMEOUT, function (event) {
            // 心跳timeout
            console.error('heart beat timeout', event);
        });
        this.pinusClient.on(omelop.WSClient.EVENT_KICK, function (event) {
            // 踢出
