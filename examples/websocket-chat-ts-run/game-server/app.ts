...
app.configure('production|development', 'connector', function () {
    ...
    // 具体看 packages/omelop/lib/components/dictionary.ts DictionaryComponentOptions
...
function errorHandler(err: Error, msg: any, resp: any,
    ...
    console.error(`${omelop.app.serverId} error handler msg[${JSON.stringify(msg)}] ,resp[${JSON.stringify(resp)}] ,
...
export function globalErrorHandler(err: Error, msg: any, resp: any,
    session: FrontendOrBackendSession, cb: HandlerCallback) {
    console.error(`${omelop.app.serverId} globalErrorHandler msg[${JSON.stringify(msg)}] ,resp[${JSON.stringify(resp)}] ,
    to resolve unknown exception: sessionId:${JSON.stringify(session.export())} ,
     error stack: ${err.stack}`);
...
app.configure('production|development', function () {
    ...
    // filter configures
    app.filter(new omelop.filters.timeout());
