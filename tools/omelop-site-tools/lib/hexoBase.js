...
class HexoBase {
  ...
  async copyFramework(docPath, targetDir) {
    ...
    // $baseDir/docs/source/languages > $targetDir/themes/omelop/languages
    ...
    if (await fs.exists(languagePath)) {
      ...
      await cpy('*.yml', path.join(targetDir, 'themes/omelop/languages'), { cwd: languagePath });
    }

    // $baseDir/docs/theme/* > $targetDir/themes/omelop
    const themePath = path.join(docPath, 'theme');
    await cpy('**/*', path.join(targetDir, 'themes/omelop'), { cwd: themePath, nodir: true, parents: true });
  }
