...
let logger = getLogger('omelop-cli', path.basename(__filename));
import * as util from '../util';
import { consts } from '../consts';
import { ICommand, AgentCommand } from '../command';
import { ReadLine } from 'readline';
import { AdminClient } from 'omelop-admin';
        ...
        client.request('watchServer', {
            ...
            if (err) console.log(err);
                ...
                if (_msg[comd]) {
                    ...
                    util.log('\ncommand \'use ' + comd + '\' error for serverId ' + comd + ' not in omelop clusters\n');
                }
            }
            rl.prompt();
        });
